...
public class SubprocessInsertion {
	...
	public static void subprocessInsertion(String basePath, String baseProcessFileName, String subProcessID, String subProcessFilePath, String resultFileName)
		...
		// Document doc2 = Utils.getDocument(subProcessFilePath,
		// subProcessID);bdfbdfbfd
		//
		// Node subProcessNode = Utils.getTAGNodeByID(doc, "bpmn2:subProcess",
		// subProcessID);
		// String incomingFlowID = Utils.getNodeFlowID(subProcessNode,
		// "bpmn2:incoming");
		// Node incomingNode = Utils.getTAGNodeByID(doc, "bpmn2:sequenceFlow",
		// incomingFlowID);
		// String outgoingFlowID = Utils.getNodeFlowID(subProcessNode,
		// "bpmn2:outgoing");
		// Node outgoingNode = Utils.getTAGNodeByID(doc, "bpmn2:sequenceFlow",
		// outgoingFlowID);
		//
		...
		// "targetRef");
		// Utils.setFlowNode(nodos.get(0), Utils.getTAGID(incomingNode),
		// "bpmn2:incoming");
		// Node ultNodoSubprocess = nodos.get(nodos.size() - 1);
		// Utils.setFlowNode(ultNodoSubprocess, outgoingFlowID,
		// "bpmn2:outgoing");
		// Utils.setFlowRef(outgoingNode, Utils.getTAGID(ultNodoSubprocess),
		// "sourceRef");
		//
		// Node incomingFlowNodeEdge = Utils.getNodeByTag(doc,
		// "bpmndi:BPMNEdge", incomingFlowID);
		// Utils.changeBPMNEdgeTarget(doc, Utils.getTAGID(incomingFlowNodeEdge),
		// Utils.getTAGID(nodos.get(0)), "targetElement");
		// Node outgoingFlowNodeEdge = Utils.getNodeByTag(doc,
		// "bpmndi:BPMNEdge", outgoingFlowID);
		// Utils.changeBPMNEdgeTarget(doc, Utils.getTAGID(outgoingFlowNodeEdge),
		// Utils.getTAGID(ultNodoSubprocess), "sourceElement");
		// Node vPIDNodeShape = Utils.getNodeByTag(doc, "bpmndi:BPMNShape",
		// Utils.getTAGID(subProcessNode));
		// Utils.figureSupression(doc, Utils.getTAGID(vPIDNodeShape),
		// "bpmndi:BPMNShape");
		//
		...
		//
		// Path target = Paths.get(basePath + File.separatorChar +
		// subProcessFilePath);
		// System.out.println("-------------- VAMOS A BORRAR EL ARCHIVO " +
		// target + " !!!");
		// Files.delete(target);
	}

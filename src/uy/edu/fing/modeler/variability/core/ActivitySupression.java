	...
	public static void activitySupression(String basePath, String baseProcessFileName, Map<String, String> selectedVariants, String resultFileName)
		...
		for (Node vPNode : vPNodes) {
			...
			if (ReemplazadorMain.DELETE.equals(selectedVariant)) {
				...
				String incomingFlow = Utils.getNodeFlowID(vPNode, "bpmn2:incoming").get(0);
				String outgoingFlow = Utils.getNodeFlowID(vPNode, "bpmn2:outgoing").get(0);

				Node incomingNode = Utils.getNodeByID(doc, incomingFlow);
				Node outgoingNode = Utils.getNodeByID(doc, outgoingFlow);
				...
				Node targetRefFinalNode = Utils.getNodeByID(doc, targetRefFinal);

...
public class BPMNLayout extends mxGraphLayout {
	 ...
	 * @param horizontal
	 */
	public BPMNLayout(Graph graph, boolean horizontal) {
	 ...
	 * @param invert
	 */
	public BPMNLayout(Graph graph, boolean horizontal, boolean invert) {
		super(graph);
	...
	protected TreeNode dfs(Object cell, Object parent, Set<Object> visited) {
		...
		GraphModel model = (GraphModel) graph.getModel();
		if (cell != null && !visited.contains(cell) && (!isVertexIgnored(cell) || model.isBoundaryEvent(cell))) {
			visited.add(cell);
	...
	protected void attachParent(TreeNode node, double height) {
		...
		if (((GraphModel) graph.getModel()).isBoundaryEvent(node.cell)) {
